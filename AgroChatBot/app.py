...
from chatbot_model import process_message, load_kb, KB_PATH, analyze_uploaded_file
...
def api_chat():
    try:
        # ✅ Handle file uploads (auto-detect)
        if "file" in request.files:
            ...
            if file and allowed_file(file.filename):
                ...
                # 🔍 Analyze the uploaded file (extract + summarize)
                summary = analyze_uploaded_file(save_path)
                if not summary:
                    summary = "Couldn't extract meaningful content from the file."
        ...
        # ✅ Handle text message
        # Support both JSON and FormData (so frontend FormData still works)
        ...
        # ✅ Process AI / offline response
        reply = process_message(user_profile, message)
        reply = sanitize_output(reply)

        # ✅ Log chat in DB
        ch = ChatHistory(user_id=user_profile["id"], user_message=message, bot_response=reply, type="text")
        db.session.add(ch)
        db.session.commit()
...
def file_analyze():
    """
    Accepts an uploaded file, analyzes it, and returns a summarized/description response.
    """
    try:
        ...
        # Store in chat history if user logged in
        if current_user.is_authenticated:
            ch = ChatHistory(
                user_id=current_user.id,
        ...
        print("Error /api/file-analyze:", e)
    ...
    try:
        with open(path,newline='',encoding='utf-8') as csvfile:
            ...
            for r in reader:
                ...
                rows.append({"keywords": keys, "answer_en": r.get('answer_en') or "", "answer_hi": r.get('answer_hi') or "", "answer_ta": r.get('answer_ta') or "", "answer_ka": r.get('answer_ka') or "", "answer_ma": r.get('answer_ma') or "", "answer_te": r.get('answer_te') or ""})
        # merge into KB
        try:
...
ALLOWED_EXT = {'png', 'jpg', 'jpeg', 'pdf', 'docx', 'txt', 'csv', 'xlsx'}
